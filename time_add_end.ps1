...
function Get-SingleKey {
    ...
    # Write-Host $k.KeyChar  # 注释，关闭按键回显，和tt.ps1统一
...
try {
    ...
    if ($hasMarkFiles.Count -gt 0) {
        ...
        $inp = Get-SingleKey "按 n 全部强制重处理；其他按键全部跳过"
        if ($inp -eq 'n') {
    ...
    # ========== 表格预览区域【修改：增加Emby状态、RENAME-ART展示、统计】 ==========
    ...
    foreach ($item in $previewTable) {
        ...
        Write-Host ("{0,-25}" -f $item.OrigFile.Name) -NoNewline
        if($item.Status -eq "RENAME"){
            Write-Host ("{0,-35}" -f $item.NewName) -ForegroundColor Yellow -NoNewline
            ...
            Write-Host ("{0,-35}" -f $item.NewName) -ForegroundColor Gray -NoNewline
    ...
    # ========== 操作选择：y改名 / s仅导出；其他键退出【修改s导出逻辑】 ==========
    $confirm = Get-SingleKey "操作选择：【y】执行改名；【s】仅生成映射txt（不改名）；其他按键退出"
    ...
    # ========== 改名完成后菜单：u撤销 / s保存映射txt / 其他退出【修改：NFO警告、日志输出】 ==========

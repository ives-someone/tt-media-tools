...
function Get-SingleKey {
    ...
    # Write-Host $k.KeyChar  # 关闭按键回显，整套工具统一
...
foreach($item in $rawList){
    ...
    if ($enableEmbyArtRename -and $ext -in $imgExts) {
        ...
        if ($null -eq $matchedVid) {
            ...
            #【新增】仅主名nfo，不带-poster/fanart这类后缀，标记待改写title
...
$previewList = $rawList
# ========== 美化预览表格【修改：增加开关提示，区分RENAME-ART的NFO-TAGS标记】 ==========
Write-Host "`n==================== 重命名预览(time_clr_begin) ====================" -ForegroundColor Cyan
